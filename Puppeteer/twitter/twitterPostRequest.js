...
async function getTwitterPost(page, searchword, pageScrollLength) {
    ...
    //var url = "https://mobile.twitter.com/search?q=diabities&src=typed_query";
...
async function logIn(page, email, password) {
    ...
    try {
        ...
        const response = await page.goto('https://mobile.twitter.com/login', {
            waitUntil: 'networkidle2'
        });
...
exports.scrapeTweets = async function(pageScrollLength, accountNo, keywords) {
    ...
    const browser = await puppeteer.launch({
        headless: isHeadless,
        args: ['--no-sandbox', '--disable-setuid-sandbox', '--enable-features=ExperimentalJavaScript', '--enable-javascript-harmony', '--lang=en-US,en;q=0.9', '--proxy-server="direct://"',
            '--proxy-bypass-list=*'
        ],
        slowMo: 10,
        userDataDir: './twitter/myUserDataDir'
    })
    ...
    var twitterLogin;
    twitterLogin = fs.readFileSync('./twitter/twitterCredentials.json');
